...
let progressClock=null,progressStart=0,lastServerMessage="";
function startProgressClock(){
 stopProgressClock();progressStart=Date.now();lastServerMessage="Submitting backtest…";
 const job=$('job');job.style.position='relative';
 let el=$('liveProgress');if(!el){el=document.createElement('div');el.id='liveProgress';el.style.cssText='margin:10px 0;font-size:13px;opacity:.85';job.appendChild(el)}
 let anim=$('liveProgressAnimation');if(!anim){anim=document.createElement('div');anim.id='liveProgressAnimation';anim.style.cssText='height:4px;overflow:hidden;background:#dce7fa;border-radius:4px;margin:8px 0';anim.innerHTML='<div style="height:100%;width:25%;background:#2463eb;border-radius:4px;animation:backtestSlide 1.3s ease-in-out infinite alternate"></div>';job.appendChild(anim)}
 if(!$('backtestSlideStyle')){let style=document.createElement('style');style.id='backtestSlideStyle';style.textContent='@keyframes backtestSlide{from{transform:translateX(0)}to{transform:translateX(300%)}}';document.head.appendChild(style)}
 progressClock=setInterval(()=>{const seconds=Math.floor((Date.now()-progressStart)/1000);el.textContent=`${lastServerMessage} · ${Math.floor(seconds/60)}m ${String(seconds%60).padStart(2,'0')}s elapsed · Still working…`},1000)
}
function stopProgressClock(){if(progressClock)clearInterval(progressClock);progressClock=null;let el=$('liveProgressAnimation');if(el)el.remove();let label=$('liveProgress');if(label)label.remove()}
$('run').onclick=async()=>{if(!cs().length)return alert('Select at least one country.');try{$('run').disabled=true;$('cached').classList.add('hide');$('job').classList.remove('hide');startProgressClock();$('jobmsg').textContent='Submitting backtest…';let j=await jf('/api/jobs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body())});$('jobid').textContent='Job '+j.job_id;poll(j.job_id)}catch(e){fail(e.message)}};
async function poll(id){clearTimeout(timer);try{let j=await jf('/api/jobs/'+id);lastServerMessage=j.message||j.status;$('jobmsg').textContent=lastServerMessage;$('pct').textContent=(j.progress||0)+'%';$('bar').style.width=(j.progress||0)+'%';if(j.status==='complete'){stopProgressClock();render(await jf('/api/jobs/'+id+'/result'),j);$('run').disabled=false;loadHistory();return}if(j.status==='failed'){fail(j.message);return}timer=setTimeout(()=>poll(id),900)}catch(e){fail(e.message)}}
function fail(m){stopProgressClock();$('jobmsg').innerHTML='<span class="error">'+esc(m)+'</span>';$('title').textContent='Backtest could not complete';$('sub').textContent=m;$('run').disabled=false}
