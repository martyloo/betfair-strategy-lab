...
async function loadFilterOptions(){
 try{
  ...
  if(!r.ok)return;let x=await r.json();
  let venue=$('venues'),current=venue.value;venue.innerHTML='<option value="">Any venue</option>'+x.venues.map(z=>`<option>${esc(z)}</option>`).join('');venue.value=current;
  let distance=$('distance'),dc=distance.value;distance.innerHTML='<option value="">Any distance</option>'+x.distances.map(z=>`<option>${esc(z)}</option>`).join('');distance.value=dc;
 }catch(e){}
}
