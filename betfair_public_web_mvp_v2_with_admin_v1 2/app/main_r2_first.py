...
class R2:
    ...
    def list(self,prefix,suffixes=(".parquet",)):
        out=[];token=None
        suffixes=tuple(x.lower() for x in suffixes) if suffixes else None
        while True:
            ...
            for x in r.get("Contents",[]):
                key=x["Key"]
                if suffixes is None or key.lower().endswith(suffixes):out.append(key)
...
def ingest(j,qd):
    ...
    try:
        admin_update(r,j,status="running",progress=1,message="Checking R2 for existing raw historical files…")
        raw_prefix=f"raw/horse-racing/win/{slug(q.plan)}/"
        raw_keys=r.list(raw_prefix,suffixes=(".bz2",".json",".jsonl",".txt"))
        st={"r2_raw_found":len(raw_keys),"r2_raw_processed":0,"requested":0,"already_processed":0,"downloaded":0,"raw_uploaded":0,"parquet_created":0,"parquet_uploaded":0,"skipped":0,"failed":0}

        # Process everything already stored in R2 first. This path does not require Betfair.
        for i,rk0 in enumerate(raw_keys,1):
            try:
                # raw keys created by raw_key() contain the stable source hash prefix.
                name=Path(rk0).name
                sid=name.split("-",1)[0] if "-" in name else hashlib.sha256(rk0.encode()).hexdigest()[:24]
                local_raw=ROOT/"admin-raw"/slug(q.plan)/name
                if not local_raw.exists() or not local_raw.stat().st_size:r.download(rk0,local_raw)
                m=parse_raw(local_raw)
                if not m:st["skipped"]+=1;continue
                try:md=datetime.fromisoformat(m.market_time.replace("Z","+00:00")).date()
                except:md=None
                if not md or not(q.from_date<=md<=q.to_date) or (m.country and m.country.upper() not in set(x.upper() for x in q.countries)):
                    continue
                pk=parquet_key(m,q.plan,sid)
                if r.exists(pk):st["already_processed"]+=1;continue
                pp=ROOT/"admin-parquet"/slug(q.plan)/(sid+".parquet");write_market(m,pp,sid,q.plan);st["parquet_created"]+=1
                r.upload(pp,pk);st["parquet_uploaded"]+=1;st["r2_raw_processed"]+=1
            except Exception:st["failed"]+=1
            if raw_keys and (i%max(1,len(raw_keys)//50)==0 or i==len(raw_keys)):
                admin_update(r,j,status="running",progress=min(70,1+int(i/len(raw_keys)*69)),message=f"Processed {i:,} of {len(raw_keys):,} raw R2 files…",stats=st)

        # Only ask Betfair for a file list after all reusable R2 raw data has been processed.
        admin_update(r,j,status="running",progress=72,message="Checking Betfair only for files not already stored in R2…",stats=st)
        ...
        for i,remote in enumerate(remotes,1):
            try:
                ...
                rk0=raw_key(remote,q.plan);raw=ROOT/"admin-raw"/slug(q.plan)/(sid+"-"+Path(str(remote)).name)
                if raw.exists() and raw.stat().st_size:pass
                elif rk0 in raw_key_set or r.exists(rk0):r.download(rk0,raw)
                else:api.download(remote,raw);st["downloaded"]+=1;r.upload(raw,rk0);raw_key_set.add(rk0);st["raw_uploaded"]+=1
                m=parse_raw(raw)
                if not m:st["skipped"]+=1;continue
                pp=ROOT/"admin-parquet"/slug(q.plan)/(sid+".parquet");write_market(m,pp,sid,q.plan);st["parquet_created"]+=1
                pk=parquet_key(m,q.plan,sid)
            ...
            if remotes and (i%max(1,len(remotes)//100)==0 or i==len(remotes)):
                admin_update(r,j,status="running",progress=min(99,72+int(i/len(remotes)*27)),message=f"Checked {i:,} of {len(remotes):,} Betfair files; downloaded only missing raw files…",stats=st)
...
def run_field_inspection(j,qd):
    ...
    try:
        ...
        keys=sorted(r.list(prefix,suffixes=(".bz2",".json",".jsonl",".txt")))
